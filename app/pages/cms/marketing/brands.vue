...
async function openModal(brand?: Brand) {
  ...
  let brandLogo: any = brand?.sm_icon;
  if (brandLogo) {
    const splitted = brandLogo.split(".");
    brandLogo = await getBlobFromUrl(brandLogo, splitted[splitted!.length - 2] || `${new Date().getTime()}`);
  }
